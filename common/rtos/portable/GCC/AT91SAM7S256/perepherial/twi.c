...
//#ifndef AT91C_TWI_CLOCK
#define AT91C_TWI_CLOCK 	100000
//#endif

//#ifndef AT91C_TWI_TMO_VOL
...
//#endif
 ...
 volatile TWI_TRANSFER_STATUS TWI_TransferStatus;
...
void AT91_TWI_ISR_ENTRY( void )  __attribute__((naked));
void AT91_TWI_ISR(void);

//*------------------------- Internal Function --------------------------------
//===============================================================================================
void AT91_TWI_ISR_ENTRY( void )
{
        portSAVE_CONTEXT();
        AT91_TWI_ISR();
        portRESTORE_CONTEXT();
...
void AT91F_TWI_Init(void)
{
        ...
        TWI_Semaphore = xSemaphoreCreateMutex();
        while( TWI_QUEUE == 0 ) ;    // Queue was not created and must not be used.
        while( TWI_Semaphore == 0 ) ;    // Queue was not created and must not be used.

        // Configure TWI in master mode
	 AT91F_TWI_Configure (AT91C_BASE_TWI);
         // Configure TWI PIOs
      	 AT91F_PIO_CfgOutput(AT91C_BASE_PIOA, AT91C_PA4_TWCK );
         AT91F_PIO_CfgOpendrain(AT91C_BASE_PIOA, // PIO controller base address
    		                ((unsigned int) AT91C_PA4_TWCK    ) |
      		                ((unsigned int) AT91C_PA3_TWD  )  );
