...
{
	AT91PS_UDP pUdp = pCdc->pUdp;
	uint packetSize, nbBytesRcv = 0, curReceiveBank = pCdc->currentRcvBank;

	while (length) {
		if ( !AT91F_UDP_IsConfigured(pCdc) )
			break;
		if ( pUdp->UDP_CSR[AT91C_EP_OUT] & curReceiveBank ) {
			...
			pUdp->UDP_CSR[AT91C_EP_OUT] &= ~(curReceiveBank);
			if (curReceiveBank == AT91C_UDP_RX_DATA_BK0)
			    curReceiveBank = AT91C_UDP_RX_DATA_BK1;
			else
			    curReceiveBank = AT91C_UDP_RX_DATA_BK0;

		}
	}
	pCdc->currentRcvBank = curReceiveBank;
	return nbBytesRcv;
